...
mkdir -p "$dest"
for t in imsg imsg-send imsg-read imsg-delete contacts contact-repair.js message-delete.swift tcc-check blip-check blip-dispatch calling_codes.py; do
  if [[ -f "$here/$t" ]]; then
...
Optional message deletion:
  Enable Accessibility for /usr/libexec/sshd-keygen-wrapper. Click +, press
  ⌘⇧G, paste that path, click Open, then enable its switch.
  Keep Messages open and the Mac unlocked. English Messages is required.
  Install Xcode Command Line Tools for Swift with xcode-select --install.
  Check prerequisites with: ~/.blip/bin/blip-check --mutations
