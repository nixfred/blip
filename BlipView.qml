...
FocusScope {
   ...
   *  WheelGlide. Touchpads (pixelDelta) stay 1:1 and immediate. The host
   *  widget's `smoothScroll` setting (shell.json) turns it off: every notch
   *  then jumps, as before. */
  property bool smoothWheel: hostWidget ? hostWidget.smoothScroll !== false : true
    ...
    Item {
      ...
      ColumnLayout {
        ...
        Flickable {
          ...
          // Touchpad scrolling is DIRECT, 1:1 — no animation: pixel deltas
          // are smooth by HARDWARE, and applying each one immediately is
          // what a browser does. A mouse-wheel notch glides (smoothWheel,
          // WheelGlide), retargeted rather than restarted, so a flood of
          // notches accumulates instead of fighting itself.
