# Blip contributors: GitHub to X

The one roster for thank-you posts. Every person with a merged PR, plus the
issue reporters who changed the code. Rebuilt from `gh pr list --state merged`
on 2026-09-25 (86 merged PRs, 21 people), then each handle checked against the
rules in CHANGE.log (`gh api users/<login>` twitter field and social accounts,
their own site, then `x user`).

**Keep it current:** add the row in the same commit as every merge. Ready-to-paste
tags are at the bottom.

## Tag these (verified)

| Person | GitHub | X | PRs | How verified |
|---|---|---|---|---|
| Ian Swope | ianswope | https://x.com/ianfs | 14 | GitHub profile + social accounts; replies to @NixFred about Omarchy. NOT @IanSwope |
| Jon Kinney | jondkinney | https://x.com/jondkinney | 12 | GitHub profile |
| Danny Cecil | jefehoser | https://x.com/jefehoser | 8 | same login on X, posts Omarchy; confirmed by Fred 2026-09-16 |
| Adam Gamble | adamgamble | https://x.com/adamgamble | 5 | GitHub profile |
...
None have published an X account (checked 2026-09-25): jacobaross, Zain Nayer
(znayer), Benny (apexbenny), Nova (nova-centauri, barrett.software).
