# Save contacts and delete messages

Blip can create a contact in the Mac's Contacts store and delete one message
through Messages. The bridge installs the helpers automatically; macOS still
...
This updates the Mac helpers, installs the Linux `contact-save` and
`imsg-delete` shims, and enrolls the confined SSH key. It preserves existing
...
3. When saving a contact for the first time, approve the **Contacts** prompt
   on the Mac. If previously denied, check **Privacy & Security → Contacts**
   for the bridge process. Full Disk Access and Contacts access are separate.
4. Keep the existing **Automation → Messages** approval used for sending.
...
prerequisites; it does not prove an individual save or deletion succeeded.
...
Contact fields and deletion requests cross bounded stdin, not command-line
