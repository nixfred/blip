...
| `~/bin/imsg`, `~/bin/imsg-send`, `~/bin/imsg-read`, `~/bin/contacts`, `~/bin/imsg-delete` | the bridge shim (a bash script) | — |
...
| Path | Contains |
|---|---|
| `~/.blip/bin/` | the bridge tools (`imsg`, `imsg-send`, `imsg-read`, `contacts`, `imsg-delete`, `tcc-check`, `blip-check`, `blip-dispatch`) |
| `~/.blip/message-delete.lock` | empty serialization lock; no message or contact data |
...
AppleScript and Accessibility. Deletion is performed by Messages itself and
verified through a read-only database query. Only message identity crosses
from Linux; text stays on the Mac during verification and is not logged. Messages.app itself keeps your
